...
const ctxOf = (text) => ({ conversationId: 't', text, lastTool: '', lastMedium: '', inDrive: false })
assert.equal(stepsFor('calendar', ctxOf('Termin morgen 15 Uhr Zahnarzt')).length, 0)
assert.ok(stepsFor('news', ctxOf('Nachrichten')).length <= RECOVER_CAP)
assert.ok(stepsFor('weather', ctxOf('Wetter heute')).length <= RECOVER_CAP)
