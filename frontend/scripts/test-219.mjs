...
assert.match(voicePluginSrc, /1400L/)
...
assert.match(voiceModeSrc, /missRef/)
...
assert.match(appSrc, /shot\.message/)
assert.match(appSrc, /setDebugHold/)
assert.match(appSrc, /capture="environment"/)
const driveSrc = readFileSync(new URL('../src/DriveMode.tsx', import.meta.url), 'utf8')
assert.match(driveSrc, /MAP_PAINT/)
assert.match(driveSrc, /#1e2329/)
assert.doesNotMatch(driveSrc, /fill="#e4c36a"/)
assert.doesNotMatch(driveSrc, /#1ed760/)
assert.match(cssSrc, /background: #e8e4d6/)
...
assert.match(devicePlugin, /grantUriPermission/)
assert.match(devicePlugin, /ACTION_IMAGE_CAPTURE/)
assert.match(devicePlugin, /RESULT_OK/)
assert.match(devicePlugin, /setDebugHold/)
const applyNative = readFileSync(new URL('./apply-native-tv.mjs', import.meta.url), 'utf8')
assert.match(applyNative, /android.media.action.IMAGE_CAPTURE/)
...
assert.equal(parseSpeakerIntent('Ich bin zu Hause'), null)
...
assert.ok(!flagReply('/hilfe', HELP_TEXT, { debug: { route: 'help', model: 'deterministic', gemini: false } }).includes('clock_without_device'))
assert.ok(!flagReply('Kompass', 'Nord, 12 Grad.', { debug: { route: 'device', model: 'deterministic', gemini: false, executed: true }, tool: { tool_status: 'executed', tool: 'device' } }).includes('weather_without_weather_tool'))
