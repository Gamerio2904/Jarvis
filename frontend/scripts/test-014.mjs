...
assert.equal(parseTvIntent(pickHeard('Fernseheren an', ['Fernseher an']))?.action, 'on')
assert.equal(parseTvIntent(normalizeUtterance('fanseher an'))?.action, 'on')
assert.equal(parseTvIntent(normalizeUtterance('fernseha an'))?.action, 'on')
assert.equal(parseTvIntent(normalizeUtterance('t v an'))?.action, 'on')
assert.equal(parseTvIntent(pickHeard('Was sagst du', ['fanseher an']))?.action, 'on')
assert.equal(pickRoute(normalizeUtterance('TV an')), 'tv')
assert.equal(pickRoute(normalizeUtterance('Mach den Fernseher an')), 'tv')
assert.equal(pickRoute(normalizeUtterance('Fernseher einschalten')), 'tv')
...
assert.match(VOICE_HINT, /1–2/)
assert.match(VOICE_HINT, /Tool/)
assert.match(VOICE_HINT, /kein Essay/)
...
assert.equal(spokenForGemini('[Hallo](https://x) _Welt_'), 'Hallo Welt')
assert.doesNotMatch(spokenForGemini('**A** _B_ [C](http://d)'), /[*_]|https/)
...
assert.ok(TEST_COPY_GROUPS.some((g) => /Welt & Lage/i.test(g.title)))
assert.ok(TEST_COPY_GROUPS.some((g) => /Naive Fragen/i.test(g.title)))
assert.ok(TEST_COPY_GROUPS.some((g) => /Screenshots/i.test(g.title)))
assert.ok(TEST_COPY_GROUPS.some((g) => /18\.8 Debug/i.test(g.title)))
...
assert.equal(preferEdgeForReply('Alles klar.'), true)
...
assert.equal(pickRoute('TV an'), 'tv')
assert.equal(pickRoute('Mach den Fernseher an'), 'tv')
assert.equal(pickRoute('Fernseher einschalten'), 'tv')
...
assert.ok(TEST_COPY_GROUPS.some((g) => /Einstellungen/i.test(g.title)))
assert.ok(TEST_COPY_GROUPS.some((g) => /Foto Notiz Dokument/i.test(g.title)))
assert.ok(TEST_COPY_GROUPS.some((g) => /Gedächtnis/i.test(g.title)))
assert.ok(TEST_COPY_GROUPS.some((g) => /Fernseher/i.test(g.title)))
assert.ok(TEST_COPY_GROUPS.some((g) => g.title === 'PC'))
assert.ok(TEST_COPY_GROUPS.some((g) => /Randfälle/i.test(g.title)))
assert.ok(TEST_COPY_GROUPS.some((g) => /Screenshots/i.test(g.title)))
assert.equal(PROBE_COPY_GROUPS.length, 13)
assert.equal(PROBE_COPY_GROUPS[0].title, 'Memory-10')
assert.equal(PROBE_COPY_GROUPS[1].title, 'Screenshots')
assert.equal(PROBE_COPY_GROUPS[9].title, 'Körper')
assert.ok(PROBE_COPY_GROUPS.every((g) => g.items.length > 0))
assert.match(HELP_TEXT, /Spur Probe/)
