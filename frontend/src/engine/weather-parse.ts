...
const NOT_WEATHER_FOLLOW =
  /\b(?:in\s+\d+\s*(?:min|minuten?|stunden?|std)|timer|wecker|termin|kalender|erinner|todo|notiz)\b/i
...
export function parseWeatherFollowup(text: string, last: WeatherLast | null): WeatherIntent | null {
  ...
  if (NOT_WEATHER_FOLLOW.test(t)) return null
  ...
  const weatherish =
    ...
    /^(in|für|aus|bei)\s+[A-Za-zÄÖÜäöüß]/.test(rest)
