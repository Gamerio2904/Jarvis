...
import { parseWontIntent, WONT_LABEL } from '../wont-parse.ts'
...
let _parseCatalog: AgentSpec[] | null = null

function buildParseCatalog(): AgentSpec[] {
  return finish([
    { id: 'wont', label: WONT_LABEL, sideEffect: 'read', parse: (ctx) => (parseWontIntent(ctx.text) ? score(ctx.text, 0.4) : null) },
    { id: 'identity', sideEffect: 'read', parse: (ctx) => (isPersonaAsk(ctx.text) ? score(ctx.text, 0.45) : null) },
    {
      id: 'tv',
      sideEffect: 'device',
      parse: (ctx) =>
        parseTvWatch(ctx.text) || parseTvIntent(ctx.text, ctx.lastTool === 'tv') || isTvDiscover(ctx.text)
          ? score(ctx.text, 0.06)
          : null,
    },
    ...
    },
    {
      ...
      parse: (ctx) =>
        ...
        parseContactsList(ctx.text) ||
        parseEmailStore(ctx.text) ||
    ...
    { id: 'calendar', sideEffect: 'write', parse: (ctx) => (parseCalendarIntent(ctx.text) ? score(ctx.text, 0.12) : null) },
    ...
    { id: 'desk', sideEffect: 'read', parse: (ctx) => (parseDeskIntent(ctx.text) ? score(ctx.text, 0.22) : null) },
    {
      id: 'eye',
      sideEffect: 'read',
      parse: (ctx) => (parseEyeIntent(ctx.text) || isEyeGround(parseGroundIntent(ctx.text)) ? score(ctx.text) : null),
    },
    { id: 'doc', sideEffect: 'read', parse: (ctx) => (parseDocIntent(ctx.text) ? score(ctx.text, 0.22) : null) },
    {
      id: 'weather',
      sideEffect: 'read',
      factual: true,
...
/** Parse-only agents — safe for route-pick without loading handlers. */
export function parseCatalog(): AgentSpec[] {
  if (!_parseCatalog) _parseCatalog = buildParseCatalog()
  return _parseCatalog
}

export function routingAgents(): AgentSpec[] {
  return parseCatalog().filter((a) => a.parse)
}
