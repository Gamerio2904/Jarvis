...
export function rewriteFollowUp(text: string, step?: LastStep | null): string | null {
  ...
  if (/^(?:call_confirm|sms_confirm|sms_body_ask|sms_ask|phone_ask|contacts_confirm|mail_confirm|mail_to_ask|mail_body_ask|wa_inbox_reply|pc_confirm|taxi|chain_ask|interrupt|cook)$/.test(tool)) return null
