...
export const BLITZER_NO_ROUTE =
  'Keine Route. Erst navigieren — dann OSM-Säulen im Korridor. Mobil bleibt leer, das jage ich nicht.'

export function parseBlitzerIntent(text: string): BlitzerIntent | null {
  const t = normalizeUtterance(text.trim())
  if (!t || t.length > 160) return null
  const cam = /\b(?:blitzer|radar(?:falle(?:n)?)?|geschwindigkeit(?:s)?kontrolle|speed.?camera)\b/i.test(t)
