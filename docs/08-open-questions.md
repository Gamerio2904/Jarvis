...
| 2026-08-15 | Q33 | **Kein Echo Show.** Jarvis läuft nicht auf Echo Show 5 (3. Gen., Rechteck, Alexa). Das Gerät ist ein Amazon-Smart-Display (Alexa/Fire-OS bzw. Vega OS), keine allgemeine Android-APK-Laufzeit. Sideload der Capacitor-App ist offiziell nicht vorgesehen; 3. Gen. (MT8169B, 5,5″ 960×480) ist geschlossen. Das ~470-MB-On-Device-Modell passt dort weder vom OS noch von RAM/Speicher. Dinge auf dem Amazon-Bildschirm anzeigen ginge nur über einen Cloud-Alexa-Skill (APL) — nicht lokal, nicht Jarvis-UI. **Werksreset, Aufschrauben, Knacken oder Code-Upload sind kein unterstützter Weg** und werden nicht dokumentiert: Reset setzt nur Alexa zurück. **Alternativen (lokal, gleiche APK):** (1) Handy, auf dem Jarvis schon läuft. (2) Altes Android-Handy oder günstiges Android-Tablet als festes Display — Sideload wie bisher, genug Speicher für ~470 MB Modell. (3) Einkaufsliste als Todo. (4) Später Samsung-TV lokal steuern (`0.11.x`) — nur Ein/Aus/Lautstärke, kein Jarvis-Chat auf dem TV. (5) TTS-Vorlesen später, PO-Kommando. Kein Fire-Tablet als Ziel (ungetestet, Amazon-OS). Bleibt Parking mit Q32/`S7.8`. Chat nennt die Alternativen. | PO-Frage + Agent |
