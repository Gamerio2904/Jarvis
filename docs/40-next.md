...
PO 2026-08-27: Reel intern nutzen — Jarvis **Hirn, Hand, Auge** und was dazugehört **sichtbar** machen, zugeschnitten auf *dieses* Jarvis. Nicht kopieren, erweitern.
...
**Die Darstellung läuft in der APK.** Dieselbe WebView-Lage wie Wetter und Akku. Kein Extra-Programm am Rechner.
...
| Hologramm / Iron-Man-Körper / Fake-Puls | Marvel, erfundene Gauges | **Won’t** |
...
| Look | Dieselben Lage-Kacheln, Spotify-dunkel. Organ-Namen deutsch, kurz. Kein Marvel-Silhouette. |
...
├────────────┬────────────┬────────────┬───────────────────┤
│  Hirn      │  Auge      │  Hand      │  Chat / Stimme    │
│  Gemini an │  Foto …    │  Taxi: Ja? │                   │
│  Friday    │  oder leer │            │                   │
├────────────┼────────────┼────────────┤                   │
│  Ohr       │  Mund      │  Gedächtnis│                   │
│  Wake aus  │  Algieba   │  12 Namen  │                   │
├────────────┴────────────┴────────────┤                   │
│  PC-Auge / PC-Hand — nur wenn BAT da, sonst ein Satz     │
└──────────────────────────────────────┴───────────────────┘
```

Wetterstatistik bleibt ein **anderes** Lage-Thema, nicht Organ. `Lage` und `Körper` sind zwei Sichten, nicht zwei Hirne.

## Researchphasen

### `4.66.0` Leitentscheidung

Dieses Dokument. **Done wenn:** APK-ja, PC-nur-Organe, Won’t-Liste, Organ-Tabelle.

### `4.67.0` Research: welche Kacheln, welche Sätze

1. Default-Körper: Hirn, Auge, Hand, Ohr, Mund, Gedächtnis. PC-Auge/PC-Hand nur wenn `pc_enabled` + Host, sonst eine Zeile.  
2. Gold: `Körper an`, `Zeig den Körper`, `Körper aus` → wieder normale Lage. `Was sieht das Auge?` bleibt Tool `eye`, nicht nur HUD.  
3. Nicht: 3D-Figur, Puls-Animation, „Organ offline“-Drama wenn Wake aus ist.  
**Done wenn:** Katalog-IDs + Default-an in der Tabelle.
...
| **`4.67.0`** | Research Kachel-Katalog | geplant |
| **`4.68.0`** | Research Live-Felder | geplant |
| **`4.69.0`** | Research PC-leer | geplant |
| **`4.70.0`** | HUD-Intent `körper` + Setting-Sicht | geplant |
| **`4.71.0`** | Kacheln Hirn / Auge / Hand | geplant |
| **`4.72.0`** | Ohr / Mund / Gedächtnis | geplant |
| **`4.73.0`** | PC-Auge / PC-Hand ehrlich | geplant |
| **`4.74.0`** | Gold, Härten, Reduced-motion | geplant |
...
Unter Tablet-Lage oder eigenes Thema **Körper**: Sicht an/aus. Hinweis: Darstellung intern, kein zweites Modell. PC-Organe brauchen JarvisPC.
...
| `frontend/src/Lage.tsx` | Organ-Kacheln, echte Snaps |
| `frontend/src/engine/hud.ts` | Snap aus Store/lastTool, kein Fake |
| Tests | `pickRoute` / parse; **nicht** `registry.ts` importieren |

## Won’t

Mails-Agent. Instagram-Poster. 24/7-Cloud-Mitarbeiter. Jarvis-Cloud. Zweites Hirn auf dem PC. Fake-Gauges. Marvel-Körper. Live-Desktop-Stream. Andere Apps auf dem Handy steuern wie ein PC-Computer-Use. Captcha-Bypass. Play Store, iOS.

## Abnahme

1. Handy ohne PC: Körper zeigt Hirn/Auge/Hand/Ohr/Mund/Gedächtnis, PC-Zeile ehrlich leer.  
2. Composer bleibt.  
3. `Lies das Foto` ändert die Auge-Kachel nach echtem Lauf, nicht vorher.  
4. Taxi-Confirm erscheint unter Hand, Satz bleibt „Bestellt habe ich nicht.“  
5. Reduced-motion ohne Puls-Spielerei.
