...
**Ist:** Code **`17.0.0`** (diese Schiene landete in `6.90.0`). Hirn **Groq primär**. `Zeig London` fliegt auf Zoom **4.4** (GIBS ab **3.8**). Stadt-Briefing + Welt-Tour **CODE**. Phrase `was ist heute so auf der Welt passiert` → `outlook` world + Tour. Glow-Scheiben am Länder-Centroid, max 5 Stops. EONET optional, bei Fehler still. **Kugel-Nachzug (Terminator, Bahn, Schichten):** [`70-next.md`](./70-next.md) §0c, Sprints 268–270.
