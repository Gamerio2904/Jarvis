...
|----|------|-------|--------|
| S248-1 | `SettingsSearchHit` Typ + Index-Builder | `settings-search-index.ts` | CODE |
| S248-2 | `SETTINGS_FIELD_INDEX` aus Labels in SettingsScreen | `SettingsScreen.tsx` | CODE |
| S248-3 | `searchSettings(q)` → Hits mit tab + field | `settings-ia.ts` | CODE |
| S248-4 | Synonyme: Fernseher→tv, Wecker→alarm, Groq→keys, … | `settings-search-index.ts` | CODE |
| S248-5 | UI: Treffer → Tab + scrollIntoView + Highlight | `SettingsScreen.tsx` | CODE |
| S248-6 | Miss-Vorschläge (Fuzzy, max 3) | `settings-ia.ts` | CODE |
| S248-7 | Tests: Fernseher, Kalender, Groq, Kugel, Löschen | unit tests | CODE |
| S248-8 | Docs: `09-versioning.md`, CHANGELOG `16.0.0` | docs | CODE |
