...
|----|------|-------|--------|
| S244-1 | CSS-Token Light: `--bg`, `--surface`, `--text`, `--accent` | `index.css` | CODE |
| S244-2 | `ui_theme` in Settings + Persist + Migration | `store.ts`, `SettingsScreen.tsx` | CODE |
| S244-3 | `data-theme` auf `<html>` oder `#root` | `App.tsx` | CODE |
| S244-4 | Theme-Transition: radial wipe von Nav-Island, 280 ms | `fx/theme-transition.ts` oder CSS | CODE |
| S244-5 | `prefers-reduced-motion` → instant swap | CSS | CODE |
| S244-6 | `prefers-color-scheme` für `system` | `App.tsx` | CODE |
| S244-7 | Lage/Kugel/Calendar lesbar in Light | `index.css`, Komponenten | CODE |
