...
Jede Folie zeigt Plakat, **Kritiker** und **Publikum**, wenn OMDb sie
liefert. Die Quelle heißt OMDb, nicht „Rotten-Tomatoes-API“. Leere
Felder bleiben leer.
...
|----|------|-------|-----------|
| S293-1 | Hit erweitern | `omdb.ts` | `OmdbHit.audience?`, `OmdbHit.poster?`. `fromOmdb`: `tomatoUserMeter` → `audience` (nicht `N/A`). `Poster` → `poster` wenn URL und nicht `N/A`. `tomatoes` bleibt Kritiker (`tomatoMeter` / Ratings) |
| S293-2 | Nachladen | `watchlist.ts` | Nach `add` oder vor Overlay-Show: `lookupOmdb(title, year)` **einmal**. Treffer auf `WatchMovie` schreiben (`critic` = `hit.tomatoes`, `audience`, `poster`, `imdbId`, `year`, `scoresAt`). Ohne Key / ohne Treffer: Felder `null`, Reply/Overlay ohne Zahl |
...
| S293-4 | Folie | `WatchlistOverlay.tsx` | Poster `<img>` nur bei `poster`. Sonst neutrale Fläche, Titel lesbar. Zwei Zeilen: `Kritiker {critic}` · `Publikum {audience}`. Fehlend: `—`. Kleine Quelle unter den Zahlen: `Rotten Tomatoes über OMDb` |
...
OMDb-Key setzen. `Watchliste: Dune` dann `Öffne Watchliste`. Folie mit
Plakat, zwei Prozent oder ehrlichem `—`. Key löschen, neuen Titel
hinzufügen: Hinweis auf Einstellungen, **keine** 87 %.
