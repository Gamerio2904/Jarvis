...
|----|------|-------|--------|
| S243-1 | `JarvisSwitch.tsx` — props: `checked`, `onChange`, `label`, `disabled` | `ui/JarvisSwitch.tsx` | PLAN |
| S243-2 | CSS: `[role=switch]`, Accent-Glow, `prefers-reduced-motion` | `index.css` | PLAN |
| S243-3 | `SettingsScreen.tsx` — alle Booleans migrieren | `SettingsScreen.tsx` | PLAN |
| S243-4 | `DebugPanel.tsx` Checkbox → Switch | `DebugPanel.tsx` | PLAN |
| S243-5 | `hud_force` Sonderfall: Switch + `setLageSession` | `SettingsScreen.tsx` | PLAN |
| S243-6 | HUD-Modul-Grid Checkboxen (falls vorhanden) | `Lage.tsx` / HUD | PLAN |
| S243-7 | a11y: `aria-checked`, Space/Enter, Focus-Ring | `JarvisSwitch.tsx` | PLAN |
