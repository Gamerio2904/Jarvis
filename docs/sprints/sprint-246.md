...
|----|------|-------|--------|
| S246-1 | Silence 800→1100 ms in VoiceMode | `VoiceMode.tsx`, `turn-detect.ts` | PLAN |
| S246-2 | Native `listenExtend` 2→4 ohne Satzende | `JarvisVoicePlugin.java` | PLAN |
| S246-3 | `repairSpeech`: inuten→Minuten, weitere STT-Artefakte | `utterance.ts` | PLAN |
| S246-4 | Partials nicht als final committen | `native/voice.ts` | PLAN |
| S246-5 | Ein Audio-Stream pro Antwort (merge TTS chunks) | `createSpeakPipeline.ts` | PLAN |
| S246-6 | Edge Timeout 1100→1800 ms Standing | `edge-tts.ts` | PLAN |
| S246-7 | Kein Lane-Wechsel mid-reply (Audit) | `native/voice.ts` | PLAN |
| S246-8 | `truncateSpoken` erst nach Satzende | `VoiceMode.tsx` | PLAN |
