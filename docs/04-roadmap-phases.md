...
**Hinweis:** Ab `0.13.0` läuft Jarvis on-device auf dem Handy. NAS/Docker/PC-Ollama sind entfallen. **`0.14`:** bestehendes härten + TV live. **`1.0`–`1.32.1`:** Alltag auf dem Handy (Wetter, Kalender, Maps, Einkauf, Losgehen, Zuhause, Auge, Sprache, Fahrmodus, Tizen-Apps). Nächste Reihe **`1.33`–`1.40`:** Qualität statt Breite — [`28-next.md`](./28-next.md). TTS ist geliefert (`1.5`+).
